...
func emptyRun(command *cobra.Command, args []string) {
	fmt.Println(args)
}
...
func init() {
	rootCmd.PersistentFlags().StringVarP(&rootflag, "r", "r", "", "")
	strtwo = rootCmd.PersistentFlags().StringP("strtwo", "t", "two", "help message for parent flag strtwo")
	...
	strthree = printCmd.PersistentFlags().StringP("strthree", "s", "three", "help message for flag strthree")
...
func main() {
	...
	fmt.Println("rootflag = " + rootflag)
	fmt.Println("strtwo = " + *strtwo)
	fmt.Println("strthree = " + *strthree)

	// go run .\example.go  -r xixi -t three print -s for
}
